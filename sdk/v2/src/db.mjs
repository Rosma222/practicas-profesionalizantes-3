 ...
 * Orden importante: primero "rol", luego "usuario", luego "permiso"
 * porque usuario depende de rol, y permiso depende de rol.
 */
export async function initDatabase(dbPath) {
  const absolutePath = resolve(dbPath);
  const db = new sqlite3.Database(absolutePath);

  // Las foreign keys en SQLite están DESACTIVADAS por defecto.
  // Hay que activarlas cada vez que se abre la base de datos.
  ...
  // Tabla permiso: cada fila es una acción permitida para un rol
  await runQuery(db, `
  ...
  // Insertamos los roles base solo si la tabla está vacía.
...
// Convierte db.run() en una Promesa para poder usar await.
// Se usa internamente en este módulo, no se exporta.
// ─────────────────────────────────────────────────────────────
...
/**
 * Alta de usuario.
 * Recibe username, password y rol_id.
 */
 ...
 * Permite cambiar username, password y/o rol_id.
 */
...
/**
 * Consulta: lista todos los usuarios.
...
/**
 * Consulta: busca un usuario por su id.
 */
...
/**
 * Consulta: busca usuario por username y password (para login).
 */
...
/**
 * Baja de rol por id.
 * Fallará si hay usuarios con ese rol asignado (protección por FK).
 */
...
/**
 * Consulta: lista todos los roles.
 */
...
/**
 * Alta de permiso: asigna una acción a un rol.
 */
...
/**
 * Consulta: lista todos los permisos de un rol específico.
 */
