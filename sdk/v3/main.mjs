...
function request_dispatcher(request, response)
{
    ...
    if (endpointsProtegidos.includes(path)) {
        ...
        // 2. Invocar al componente Autorizador con la nueva estructura de tablas (Punto 1)
        const autorizado = comprobar_permiso_real(username, path);

        if (!autorizado) {
            // No tiene vinculada la ruta en la tabla 'access'. Mandamos código controlado del cliente.
